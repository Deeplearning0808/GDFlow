...
def load_MSL_SMAP(data_name, batch_size, window_size, stride_size, train_split):
    if data_name == 'MSL':
        chan_id = 'P-15'
    elif data_name == 'SMAP':
        chan_id = 'T-3'
    else:
        raise ValueError("Invalid data_name. Expected 'MSL', 'SMAP'.")
    print('chan_id:', chan_id)

    base_dir = f"./benchmarks/{data_name}"
    ...
    T, C = test_X.shape
    if data_name == 'MSL':
        MSL_SMAP_label = label[label["spacecraft"] == "MSL"].sort_values(by="chan_id")
    elif data_name == 'SMAP':
        MSL_SMAP_label = label[label["spacecraft"] == "SMAP"].sort_values(by="chan_id")
    anos = ast.literal_eval(MSL_SMAP_label[MSL_SMAP_label["chan_id"] == chan_id]["anomaly_sequences"].values[0])
    ...
    train_dataset = MSL_SMAP_dataset(trainset, window_size, stride_size)
    if train_split != 1.0:
        val_dataset = MSL_SMAP_dataset(valset, window_size, stride_size)
    test_dataset = MSL_SMAP_dataset(testset, window_size, stride_size, test_label)
...
class MSL_SMAP_dataset(Dataset):
    def __init__(self, df, window_size, stride_size, labels=None) -> None:
        super(MSL_SMAP_dataset, self).__init__()
        self.df = df
